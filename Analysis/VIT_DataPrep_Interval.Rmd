Analysis of elk abortions 2014
========================================================
...
Let's look at the time to VIT recovery
```{r vitrecovery}
# interval when the events are known to occur
quantile(Data$interval2, probs = seq(0,1,.05), na.rm = T)
# interval between when the VIT is known to be out and it was recovered on the ground
quantile(Data$RecoveryInterval, probs = seq(0,1,.05), na.rm = T)
# maximum possible time out
max.time <- Data$RecoveryInterval + Data$interval2
quantile(max.time, probs = seq(0,1,.05), na.rm = T)
par(mfrow = c(1,2))
hist(Data$interval2)
hist(Data$RecoveryInterval)
...
# remove the rows where no event occurs and the interval == 0. 
...
```{r}
rm(i,j,tmp)
save.image(file = 'VIT_Data2014_Full.RData')
```
...
save.image(file = 'VIT_Data2014_Full_POS.RData')
```
Subset to abortions and censored birth events
-------------------------------------------------------------------------------
```{r 3rd-subset}
Data <- OldData  # restore the old data
Data <- subset(Data, Data$CowSero == "POS") # limit it to seropositive only. 
Data <- Data[order(Data$Elk_ID, Data$CapDOY),] # sort it
n <- dim(Data)[1] # number of elk
...
# remove rows where births occur (these are censored in this subset)
tmp <- which(Data$event == 1 & Data$VIT_Status == "Parturition") 
Data <- Data[-tmp,]

#recode left and right to be 1 to max
Data$right <- Data$right - min(Data$left) + 1
Data$left  <- Data$left - min(Data$left) + 1
...
save.image(file = 'VIT_Data2014_BirthsCensored.RData')
```
