...
table(year(Data$CaptureDate), Data$VIT_Status)
# Table of data quality by year
table(year(Data$CaptureDate), Data$TrustRating)
# Table of event types by trust rating
table(Data$VIT_Status, Data$TrustRating)
# Look at the Unknowns
Data[Data$VIT_Status == "Unknown",]
```
...
```{r}
rm(i,j,tmp)
save.image(file = 'VIT_Data2013_Full.RData')
```
...
save.image(file = 'VIT_Data2013_Full_POS.RData')
```
...
# remove rows where nothing happens on an interval == 0
Data <- subset(Data, Data$interval != 0) 
...
save.image(file = 'VIT_Data2013_BirthsCensored.RData')
```
